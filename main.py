...
###############################################################################
# markdown2Mathjax
# Unfortunately, using `pip install markdown2Mathjax` does not work. 
# https://github.com/constantAmateur/markdown2Mathjax/issues/1
# Hence, I just copied in the source code. 
# All credit goes to https://github.com/constantAmateur
###############################################################################

import re
from markdown2 import markdown

def markdown2Mathjax_break_tie(inline,equation):
...
def markdown2Mathjax_markdown_safe(placeholder):
...
def markdown2Mathjax_mathdown(text):
    """Convenience function which runs the basic markdown and mathjax processing sequentially."""
    tmp = markdown2Mathjax_sanitizeInput(text)
    return markdown2Mathjax_reconstructMath(markdown(tmp[0]),tmp[1])

def markdown2Mathjax_sanitizeInput(string,inline_delims = ["$","$"],equation_delims = ["$$","$$"],placeholder = "$0$"):
    ...
    if not markdown2Mathjax_markdown_safe(placeholder):
    ...
    while 1:
        ...
        if startmatches[0] and min(startpoints) == startpoints[0]:
            ...
            if not inBlock:
                inBlock  =  markdown2Mathjax_break_tie(startmatches[1],startmatches[2])
        #Magic to ensure minimum index is 0
        sanitizedString  =  sanitizedString + string[(post if post > 0 else 0):startpoints[inBlock]]
...
def markdown2Mathjax_reconstructMath(processedString,codeblocks,inline_delims = ["$","$"],equation_delims = ["$$","$$"],placeholder = "$0$",htmlSafe = False):
...
class File:
    ...
    def _get_html_data(self) -> dict:
        ...
        html_content = markdown2.markdown(
            markdown2Mathjax_sanitizeInput(prepared_md_content)[0],
        ...
        html_content = markdown2Mathjax_reconstructMath(html_content, markdown2Mathjax_sanitizeInput(prepared_md_content)[1])
